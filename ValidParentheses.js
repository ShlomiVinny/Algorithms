...
var isValid = function(s) {
    ...
    const Values = {                // this is the dictionary for the parentheses, each closing parenthes is the [key] while the openning parenthes are the [value]
     ...
     // this will be our stack, since js doesnt implement a Stack object, i'll just use an array and only use the correct methods that fit a stack,
    // such as: push() and pop() to keep the First In - First Out nature of a stack
    let stack = [];
    ...
    for (let i = 0; i < s.length; i++) {                      // loop to itterate over String s
        let char = s.charAt(i);                               // get the char an index <i> from String s
        let contains = Values.hasOwnProperty(char);           // this method checks if Values has char as a [key]
        if (!contains) {                                      // if char is not a [key] in Values:
            ...
            if (Values[char] !== last) {                      // if the [value] that meats the [key] in Values, does not equal to the last value from the stack, then:
                isValid = false;                              // this string is not valid, set isValid to false
                break;                                        // break out of the loop
            }
        } else if (contains && stack.length === 0) {          // otherwise, if the parenthes is a [key] in Values and the stack is empty,
                                                              // then theres no matching parenthes for it, then:
            isValid = false;                                  // set isValid to false, and
            break;                                            // break out of the loop
        }
    }
