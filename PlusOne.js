...
var plusOne = function(digits) {
    ...
    let i = -1; // index of -1 is equivalent to digits[digits.length - 1]
    ...
    while (true) {
        if (lastDigit === 9) { // if the last digit is 9, we need to zero it out and add one to the next digit
            ...
            continue; // rerun loop 
        } else if (lastDigit === undefined) { // if the digit does not exist we get an undef, therefore we must expand the array and add another member at the start (index = 0)
            digits.unshift(0); // unshift a 0 which will be turned to a 1 in the next line
        }
        //else if lastDig is not a 9 and is not undef, just add a 1
